...
export function AnimeCard({
  ...
  const topRight = epNum != null ? (
    <span className="absolute right-2 top-2 rounded-md bg-frost-soft px-1.5 py-0.5 text-[10px] font-bold text-frost backdrop-blur-sm">
      EP {epNum}
    </span>
  ) : (
    !!anime.score && (
      <span className="absolute right-2 top-2 flex items-center gap-0.5 rounded-md bg-base/75 px-1.5 py-0.5 text-[10px] font-bold text-snow backdrop-blur-sm">
  ...
  if (CARD_DESIGN === "cinematic") {
    return (
      <Link href={`/anime/${anime.id}`} className={cn("group block snap-start", className)}>
        <div className="relative aspect-[3/4] overflow-hidden rounded-[1.25rem] bg-surface-2 shadow-[var(--shadow-soft)] ring-1 ring-line/80 transition-all duration-300 ease-out group-hover:-translate-y-1.5 group-hover:ring-frost/50 group-hover:shadow-[var(--shadow-frost)]">
          ...
          {/* Bottom scrim — always present for legibility, deepens on hover */}
          <div className="absolute inset-x-0 bottom-0 h-3/5 bg-gradient-to-t from-base via-base/55 to-transparent transition-all duration-300 group-hover:via-base/75" />
            ...
            <div className="grid grid-rows-[0fr] opacity-0 transition-all duration-300 ease-out group-hover:grid-rows-[1fr] group-hover:opacity-100">
              <div className="overflow-hidden">
                <div className="flex items-center gap-2 pt-2.5">
                  ...
                  {anime.type && (
                    <span className="rounded-full border border-white/10 bg-white/[0.06] px-2.5 py-1 text-[11px] font-semibold text-snow/90 backdrop-blur-sm">
                      {anime.type}
                    </span>
                  )}
                  {!!anime.score && (
